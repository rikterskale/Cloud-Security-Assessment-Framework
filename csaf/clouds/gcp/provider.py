"""GCP provider: orchestrates read-only modules at project scope.

Google Cloud list APIs are project-wide (aggregated across zones/regions where
needed), so all modules are evaluated once at "global" scope.
"""
...
class GcpProvider:
    def __init__(self, session, baseline, evidence, logger, engagement, profile: str):
        ...
        self.account_id = session.project_id
    ...
    def evaluate(self, controls: list[Control], regions: list[str]) -> list:
        ...
        for module_name, module_controls in grouped.items():
            if module_name == "attestation":
                results.extend(attestation_results(module_controls, self.engagement, "GCP", self.account_id))
                continue
            ...
            for control in module_controls:
                self.logger.info(module_name, f"Evaluating {control.id} (project)", controlId=control.id)
